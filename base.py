...
def create_scrubbing_preproc():
    ...
    sc_NumFD = pe.MapNode(util.Function(input_names=['infile_a', 'infile_b'],
                                        output_names=['out_file'],
                          function=setNumFD),
                          name='sc_NumFD',
                          iterfield=['infile_a', 'infile_b'])

    sc_PercentFD = pe.MapNode(util.Function(input_names=['infile_a',
                                                        'infile_b'],
                                            output_names=['out_file'],
    ...
    sc_FramesEx = pe.MapNode(util.Function(input_names=['in_file'],
                                           output_names=['out_file'],
                             function=setFramesEx),
                             name='sc_FramesEx',
                             iterfield=['in_file'])

    sc_FramesIN = pe.MapNode(util.Function(input_names=['in_file'],
                                           output_names=['out_file'],
    ...
    sc_MeanDVARS = pe.MapNode(util.Function(input_names=['infile_a',
                                            'infile_b'],
                                            output_names=['out_file'],
    ...
    sc_NUM5 = pe.MapNode(util.Function(input_names=['infile_a',
                                       'infile_b'],
                                       output_names=['out_file'],
                         function=setNUM5),
                         name='sc_NUM5',
    ...
    sc_NUM10 = pe.MapNode(util.Function(input_names=['infile_a',
                                        'infile_b'],
                                        output_names=['out_file'],
                          function=setNUM10),
                          name='sc_NUM10',
    ...
    sc_NUMFD = pe.MapNode(util.Function(input_names=['in_file'],
                                        output_names=['out_file'],
                          function=setNUMFD),
                          name='sc_NUMFD',
                          iterfield=['in_file'])

    sc_ScrubbedMotion = pe.MapNode(util.Function(input_names=['infile_a',
                                                 'infile_b'],
                                                 output_names=['out_file'],
    ...
    sc_FtoFPercentChange = pe.MapNode(util.Function(input_names=['infile_a',
                                                    'infile_b'],
                                                    output_names=['out_file'],
    ...
    sc_SqrtMeanDeriv = pe.MapNode(util.Function(input_names=['in_file'],
                                                output_names=['out_file'],
    ...
    sc.connect(inputNode, 'rest',
               sc_calc1, 'infile_b')
    sc.connect(NVOLS, ('nvols', last_vol),
               sc_calc1, 'stop_idx')
    sc.connect(NVOLS, ('nvols', TRendminus1),
               sc_calc1, 'stop_idx2')

    sc.connect(sc_calc1, 'brik_file',
                sc_calc2, 'infile_a')
    ...
    sc.connect(sc_automask, 'brik_file',
               sc_3dROIstats_1, 'mask')
    sc.connect(sc_calc2, 'brik_file',
               sc_3dROIstats_1, 'in_file')

    sc.connect(sc_automask, 'brik_file',
               sc_3dROIstats_2, 'mask')
    sc.connect(sc_calc3, 'brik_file',
               sc_3dROIstats_2, 'in_file')

    sc.connect(sc_3dROIstats_1, 'stats',
               sc_SqrtMeanDeriv, 'in_file')
    sc.connect(sc_3dROIstats_2, 'stats',
               sc_SqrtMeanRaw, 'in_file')

    sc.connect(sc_SqrtMeanDeriv, 'out_file',
               sc_FtoFPercentChange, 'infile_a')
    sc.connect(sc_SqrtMeanRaw, 'out_file',
               sc_FtoFPercentChange, 'infile_b')

    ###Calculating mean Framewise Displacement
    sc.connect(inputNode, 'movement_parameters',
               sc_createSC, 'in_file')
    sc.connect(sc_createSC, 'out_file',
               sc_MeanFD, 'infile_b')
    sc.connect(sc_copy, 'out_file',
               sc_MeanFD, 'infile_a')

    ##NUMBER OF FRAMES >0.5mm FD
    sc.connect(sc_MeanFD, 'out_file',
               sc_NumFD, 'infile_a')
    sc.connect(sc_createSC, 'out_file',
               sc_NumFD, 'infile_b')

    ##NUMBER OF FRAMES >0.5mm FD as percentage of total num frames
    sc.connect(sc_NumFD, 'out_file',
               sc_PercentFD, 'infile_a')
    sc.connect(sc_createSC, 'out_file',
               sc_PercentFD, 'infile_b')

    ####Mean DVARS
    sc.connect(sc_PercentFD, 'out_file',
               sc_MeanDVARS, 'infile_a')
    sc.connect(sc_FtoFPercentChange, 'out_file',
               sc_MeanDVARS, 'infile_b')

    ###NUMBER OF relative FRAMES >5%
    sc.connect(sc_MeanDVARS, 'out_file',
               sc_NUM5, 'infile_a')
    sc.connect(sc_FtoFPercentChange, 'out_file',
               sc_NUM5, 'infile_b')

    ###NUMBER OF relative FRAMES >10%
    sc.connect(sc_NUM5, 'out_file',
               sc_NUM10, 'infile_a')
    sc.connect(sc_FtoFPercentChange, 'out_file',
               sc_NUM10, 'infile_b')
    ...
    ## but because images start at 0,
    ##it's ok to take the frame number directly from the FD file
    ## (i.e., if the 5th number in the FD file indicates a bad frame,
    ##removing timepoint '5' will correctly remove the 6th frame).
    sc.connect(sc_createSC, 'out_file',
               sc_FramesEx, 'in_file')
    sc.connect(sc_createSC, 'out_file',
               sc_FramesInList, 'in_file')
    sc.connect(sc_createSC, 'out_file',
               sc_FramesIN, 'in_file')

    sc.connect(inputNode, 'rest',
               sc_calc_scrub, 'infile_a')
    sc.connect(sc_FramesIN, ('out_file', getStartIdx),
               sc_calc_scrub, 'start_idx')
    sc.connect(sc_FramesIN, ('out_file', getStopIdx),
               sc_calc_scrub, 'stop_idx')

    sc.connect(inputNode, 'movement_parameters',
               sc_ScrubbedMotion, 'infile_b')
    sc.connect(sc_FramesInList, 'out_file',
               sc_ScrubbedMotion, 'infile_a')

    sc.connect(sc_3dROIstats_1, 'stats',
               outputNode, 'mean_deriv_sq_1D')
    sc.connect(sc_3dROIstats_2, 'stats',
               outputNode, 'mean_raw_sq_1D')
    sc.connect(sc_calc_scrub, 'out_file',
               outputNode, 'scrubbed_preprocessed')
    sc.connect(sc_calc1, 'brik_file',
               outputNode, 'temp_deriv_brik_file')
    sc.connect(sc_calc1, 'head_file',
               outputNode, 'temp_deriv_head_file')
    sc.connect(sc_calc2, 'brik_file',
               outputNode, 'temp_deriv_sq_brik_file')
    sc.connect(sc_calc2, 'head_file',
               outputNode, 'temp_deriv_sq_head_file')
    sc.connect(sc_calc3, 'brik_file',
               outputNode, 'raw_sq_brik_file')
    sc.connect(sc_calc3, 'head_file',
               outputNode, 'raw_sq_head_file')
    sc.connect(sc_automask, 'brik_file',
               outputNode, 'mask_brik_file')
    sc.connect(sc_automask, 'head_file',
               outputNode, 'mask_head_file')
    sc.connect(sc_createSC, 'out_file',
               outputNode, 'FD_1D')
    sc.connect(sc_SqrtMeanDeriv, 'out_file',
               outputNode, 'sqrt_mean_deriv_sq_1D')
    sc.connect(sc_SqrtMeanRaw, 'out_file',
               outputNode, 'sqrt_mean_raw_sq_1D')
    sc.connect(sc_FramesEx, 'out_file',
               outputNode, 'frames_ex_1D')
    sc.connect(sc_FramesIN, 'out_file',
               outputNode, 'frames_in_1D')
    sc.connect(sc_NUM10, 'out_file',
               outputNode, 'pow_params')
    sc.connect(sc_FtoFPercentChange, 'out_file',
               outputNode, 'ftof_percent_change_1D')
    sc.connect(sc_ScrubbedMotion, 'out_file',
               outputNode, 'scrubbed_movement_parameters')
...
def create_group_analysis(f_test):
    ...
    gp_fslmerge = pe.Node(interface=fsl.Merge(),
                          name='gp_fslmerge')
    gp_fslmerge.inputs.dimension = 't'

    gp_fslmaths = pe.Node(interface=fsl.ImageMaths(),
                          name='gp_fslmaths')
    gp_fslmaths.inputs.op_string = '-abs -Tmin -bin'

    gp_flameo = pe.Node(interface=fsl.FLAMEO(),
                        name='gp_flameo')
    ...
    gp_fslcpgeom = pe.MapNode(util.Function(input_names=['infile_a',
                                                'infile_b'],
                                            output_names=['out_file'],
                              function=copyGeom),
                              name='gp_fslcpgeom',
    ...
    gp_merge = pe.MapNode(util.Function(input_names=['infile_a',
                                        'infile_b'],
                                        output_names=['out_file'],
                          function=getTuple),
                          name='gp_merge',
    ...
    gp_slicer = pe.MapNode(interface=fsl.Slicer(),
                           name='gp_slicer',
                           iterfield=['in_file'])
    gp_slicer.inputs.image_width = 750
    gp_slicer.inputs.all_axial = True

    gp_fslmultimaths2 = pe.Node(interface=fsl.ImageMaths(),
                        name='gp_fslmaths2')
    ...
    gp_getbackgroundimage = pe.MapNode(util.Function(input_names=['in_file'],
                            ...
                            name='gp_getbackgroundimage',
                            iterfield=['in_file'])

    grp_analysis.connect(inputnode, 'seed_files',
                         gp_fslmerge, 'in_files')
    grp_analysis.connect(gp_fslmerge, 'merged_file',
                         gp_fslmaths, 'in_file')

    if f_test:
        grp_analysis.connect(gp_fslmerge, 'merged_file',
                             gp_flameo, 'cope_file')
        grp_analysis.connect(gp_fslmaths, 'out_file',
                             gp_flameo, 'mask_file')
        grp_analysis.connect(inputnode, 'mat_file',
                             gp_flameo, 'design_file')
        grp_analysis.connect(inputnode, 'con_file',
                             gp_flameo, 't_con_file')
        grp_analysis.connect(inputnode, 'grp_file',
                             gp_flameo, 'cov_split_file')
        grp_analysis.connect(inputnode, 'fts_file',
                             gp_flameo, 'f_con_file')
    else:
        grp_analysis.connect(gp_fslmerge, 'merged_file',
                             gp_flameo, 'cope_file')
        grp_analysis.connect(gp_fslmaths, 'out_file',
    ...
    grp_analysis.connect(gp_flameo, 'zstats',
                         gp_smooth_estimate, 'zstat_file')
    grp_analysis.connect(gp_fslmaths, 'out_file',
                         gp_smooth_estimate, 'mask_file')
    grp_analysis.connect(gp_flameo, 'zstats',
                         gp_fslmultimaths, 'in_file')
    grp_analysis.connect(gp_fslmaths, 'out_file',
                         gp_fslmultimaths, 'operand_files')
    grp_analysis.connect(gp_fslmultimaths, 'out_file',
                         gp_getbackgroundimage, 'in_file')
    grp_analysis.connect(gp_getbackgroundimage, 'out_file',
                         gp_fslcpgeom, 'infile_a')
    grp_analysis.connect(gp_fslmultimaths, 'out_file',
                         gp_fslcpgeom, 'infile_b')
    grp_analysis.connect(gp_fslcpgeom, 'out_file',
                         gp_cluster, 'in_file')
    grp_analysis.connect(inputnode, 'z_threshold',
                         gp_cluster, 'threshold')
    grp_analysis.connect(inputnode, 'p_threshold',
                         gp_cluster, 'pthreshold')
    grp_analysis.connect(gp_smooth_estimate, 'volume',
                         gp_cluster, 'volume')
    grp_analysis.connect(gp_smooth_estimate, 'dlh',
                         gp_cluster, 'dlh')
    grp_analysis.connect(gp_cluster, 'threshold_file',
                         gp_fslstats, 'in_file')
    grp_analysis.connect(gp_fslstats, 'out_stat',
                         gp_merge, 'infile_b')
    grp_analysis.connect(inputnode, 'z_threshold',
                         gp_merge, 'infile_a')

    grp_analysis.connect(gp_cluster, 'threshold_file',
                         gp_overlay, 'stat_image')
    grp_analysis.connect(gp_merge, 'out_file',
                         gp_overlay, 'stat_thresh')

    grp_analysis.connect(gp_fslmaths,
                        ('out_file', get_standard_background_img),
                         gp_overlay, 'background_image')

    grp_analysis.connect(gp_overlay, 'out_file',
                         gp_slicer, 'in_file')

    grp_analysis.connect(gp_fslmerge, 'merged_file',
                         gp_nvols, 'in_file')
    grp_analysis.connect(gp_fslmerge, 'merged_file',
                         gp_fslmultimaths2, 'in_file')
    grp_analysis.connect(gp_nvols, 'out_file',
                         gp_fslmultimaths2, 'op_string')

    grp_analysis.connect(gp_fslmerge, 'merged_file',
                         outputnode, 'merged')
    grp_analysis.connect(gp_fslmaths, 'out_file',
                         outputnode, 'math_mask')
    grp_analysis.connect(gp_flameo, 'zstats',
                         outputnode, 'zstats')
    grp_analysis.connect(gp_fslmultimaths, 'out_file',
                         outputnode, 'multi_math_mask')
    grp_analysis.connect(gp_fslcpgeom, 'out_file',
                         outputnode, 'orig_threshold')
    grp_analysis.connect(gp_smooth_estimate, 'volume',
                         outputnode, 'volume')
    grp_analysis.connect(gp_smooth_estimate, 'dlh',
                         outputnode, 'degree_of_freedom')
    grp_analysis.connect(gp_cluster, 'threshold_file',
                         outputnode, 'cluster_threshold')
    grp_analysis.connect(gp_cluster, 'localmax_txt_file',
                         outputnode, 'cluster_localmax')
    grp_analysis.connect(gp_cluster, 'index_file',
                         outputnode, 'cluster_index')
    grp_analysis.connect(gp_overlay, 'out_file',
                         outputnode, 'combined_image_volume')
    grp_analysis.connect(gp_merge, 'out_file',
                         outputnode, 'min_max_thresh')
    grp_analysis.connect(gp_slicer, 'out_file',
                         outputnode, 'picture')
    grp_analysis.connect(gp_fslmultimaths2, 'out_file',
                         outputnode, 'multi_math_mask2')
...
def create_timeseries_preproc(unit_time_series_extraction,
                              voxel_time_series_extraction,
                              vertices_time_series_extraction):
    ...
    inputnode_getparc = pe.Node(util.IdentityInterface(
                                    fields=['parcelations']),
                                name='getparc')

    inputNode_getmask = pe.Node(util.IdentityInterface(
                                    fields=['masks']),
                                name='getmask')

    timeseries_reconall = pe.Node(interface=fs.ReconAll(),
                                  name='timeseries_reconall')
    timeseries_reconall.inputs.directive = 'all'

    timeseries_bbreg = pe.MapNode(interface=fs.BBRegister(init='fsl',
                                                  contrast_type='t2',
                                                  registered_file=True,
                                                  out_fsl_file=True),
                                  name='timeseries_bbreg',
                                  iterfield=['source_file'])

    timeseries_apply_warp = pe.MapNode(interface=fsl.ApplyWarp(),
                                       name='timeseries_apply_warp',
                                       iterfield=['in_file',
                                       'premat'])

    timeseries_sampler_lh = pe.MapNode(interface=fs.SampleToSurface(hemi='lh'),
                                       name='timeseries_sampler_lh',
                                       iterfield=['source_file',
                                       'reg_file'])
    ...
    timeseries_sampler_rh = pe.MapNode(interface=fs.SampleToSurface(hemi='rh'),
                                       name='timeseries_sampler_rh',
    ...
    timeseries_flirt = pe.MapNode(interface=fsl.FLIRT(),
                                  name='timeseries_flirt',
                                  iterfield=['in_file'])
    ...
    timeseries_gen_parc = pe.MapNode(util.Function(input_names=['data_file',
                                                    'template',
                                                    'unitTSOutputs'],
                                                    output_names=['out_file'],
    ...
    timeseries_gen_mask = pe.MapNode(util.Function(input_names=['data_file',
                                                    'template',
                                                    'voxelTSOutputs'],
                                                    output_names=['out_file'],
    ...
    timeseries_gen_surface = pe.MapNode(util.Function(
                                    input_names=['rh_surface_file',
                                        'lh_surface_file',
                                        'verticesTSOutputs'],
                                    output_names=['out_file'],
                                    function=gen_csv_for_surface),
                                    name='timeseries_gen_surface',
                                    iterfield=['rh_surface_file',
                                            'lh_surface_file'])

    preproc.connect(inputNode, 'motion_correct',
                    timeseries_apply_warp, 'in_file')
    preproc.connect(inputNode, 'warp_file',
                    timeseries_apply_warp, 'field_file')
    preproc.connect(inputNode, 'premat',
                    timeseries_apply_warp, 'premat')
    preproc.connect(inputNode, 'standard',
                    timeseries_apply_warp, 'ref_file')

    """
        Surface Registration turn off/on
    """
    preproc.connect(inputNode, 'brain',
                    timeseries_reconall, 'T1_files')
    preproc.connect(inputNode, ('reorient', extract_subjectID),
                    timeseries_reconall, 'subject_id')
    preproc.connect(inputNode, 'recon_subjects',
                    timeseries_reconall, 'subjects_dir')

    preproc.connect(timeseries_apply_warp, 'out_file',
                    timeseries_bbreg, 'source_file')
    preproc.connect(timeseries_reconall, 'subjects_dir',
                    timeseries_bbreg, 'subjects_dir')
    preproc.connect(timeseries_reconall, 'subject_id',
                    timeseries_bbreg, 'subject_id')

    preproc.connect(timeseries_bbreg, 'out_reg_file',
                    timeseries_sampler_lh, 'reg_file')
    preproc.connect(timeseries_apply_warp, 'out_file',
                    timeseries_sampler_lh, 'source_file')

    preproc.connect(timeseries_bbreg, 'out_reg_file',
                    timeseries_sampler_rh, 'reg_file')
    preproc.connect(timeseries_apply_warp, 'out_file',
                    timeseries_sampler_rh, 'source_file')
    ...
    if unit_time_series_extraction:
        preproc.connect(timeseries_apply_warp, 'out_file',
                        timeseries_flirt, 'in_file')
        preproc.connect(inputNode, 'identity_matrix',
                        timeseries_flirt, 'in_matrix_file')
        preproc.connect(inputnode_getparc, 'parcelations',
                        timeseries_flirt, 'reference')

        preproc.connect(timeseries_flirt, 'out_file',
                        timeseries_gen_parc, 'data_file')
        preproc.connect(inputNode, 'unitTSOutputs',
                        timeseries_gen_parc, 'unitTSOutputs')
        preproc.connect(inputnode_getparc, 'parcelations',
                        timeseries_gen_parc, 'template')

    if voxel_time_series_extraction:

        preproc.connect(timeseries_apply_warp, 'out_file',
                        timeseries_flirt1, 'in_file')
        preproc.connect(inputNode, 'identity_matrix',
                        timeseries_flirt1, 'in_matrix_file')
        preproc.connect(inputNode_getmask, 'masks',
                        timeseries_flirt1, 'reference')

        preproc.connect(timeseries_flirt1, 'out_file',
                        timeseries_gen_mask, 'data_file')
        preproc.connect(inputNode, 'voxelTSOutputs',
                        timeseries_gen_mask, 'voxelTSOutputs')
        preproc.connect(inputNode_getmask, 'masks',
                        timeseries_gen_mask, 'template')

    if vertices_time_series_extraction:

        preproc.connect(timeseries_sampler_rh, 'out_file',
                        timeseries_gen_surface, 'rh_surface_file')
        preproc.connect(timeseries_sampler_lh, 'out_file',
                        timeseries_gen_surface, 'lh_surface_file')
        preproc.connect(inputNode, 'verticesTSOutputs',
                        timeseries_gen_surface, 'verticesTSOutputs')
