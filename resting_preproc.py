...
def getGroupAnalysisInputs(c):
    ...
    try:
        ...
        f = open(c.derivativeFile, 'r')
        derivatives = f.readlines()
        f.close()

        for drv in derivatives:
            drv = os.path.basename(drv.rstrip('\r\n'))
            drv = os.path.splitext(os.path.splitext(drv)[0])[0]
            dervlist.append(drv)

        print 'checking for derivatives ', dervlist
        
        template_dict = dict(mat = c.mat,
                             con = c.con,
                             fts = c.fts,
                             grp = c.grp,
    ...
    return  modelist, dervlist, template_dict, template_args
...
#                     vmhcpreproc, 'inputspec.example_func2highres_mat')
    ...
    workflow.connect(flowAnatFunc, 'inputnode.subject', datasink, 'container')
...
#    timeseries_sink(workflow,datasink, tspreproc, c.runSurfaceRegistraion[0])
